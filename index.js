...
async function run() {
  ...
  packets.forEach(packet => console.log(packet));
...
function readPacket(start, content) {
  ...
  readSpacePacketPrimaryHeader(packet);
  readSpacePacketSecondaryHeader(packet);
...
function readSpacePacketPrimaryHeader(packet) {
  let startIndex = (packet.M_PDU_HEADER.FIRST_HEADER_POINT*2) + 4;
  let headerLengthHex = 12;
  packet.SPACE_PACKET_PRIMARY_HEADER_HEX = packet.data.substr(startIndex, headerLengthHex);
  let header = Buffer.from(packet.SPACE_PACKET_PRIMARY_HEADER_HEX, 'hex').readUIntBE(0, 6);
  ...
  packet.SPACE_PACKET_PRIMARY_HEADER = headerValues;

  let dataStart = startIndex + headerLengthHex + (8*2);
  let dataEnd = dataStart + (headerValues.PACKET_DATA_LENGTH * 2);
  packet.SPACE_PACKET_START = startIndex;
  packet.SPACE_PACKET_DATA_START = dataStart;
  packet.SPACE_PACKET_DATA_END = dataEnd;
  packet.DATA_LENGTH = packet.data.length;
  if( dataEnd > packet.data.length ) {
    packet.DATA_OVERFLOW = dataEnd - packet.data.length;
  }
}

function readSpacePacketSecondaryHeader(packet) {
  let startIndex = (packet.M_PDU_HEADER.FIRST_HEADER_POINT*2) + 4 + 12;
  let headerLengthHex = 8*2;
  packet.SPACE_PACKET_SECONARY_HEADER_HEX = packet.data.substr(startIndex, headerLengthHex);
  ...
  packet.SPACE_PACKET_SECONDARY_HEADER = headerValues;
}
