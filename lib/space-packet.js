...
class SpacePacket {
  ...
  parse(startIndex, data) {
    ...
    if( this.packet.PRIMARY_HEADER_NOT_FULLY_SET ) {
      ...
      this.packet.PRIMARY_HEADER = Buffer.concat([this.packet.PRIMARY_HEADER, data.slice(startIndex, startIndex+missingLength)]);
      
      // this is setting up the computation below where figure out the start
      // index of the data segment
      startIndex = startIndex + missingLength - CONSTANTS.SPACE_PACKET.DEF.PRIMARY_HEADER;  
  ...
  _parsePrimaryHeader() {
    ...
    this.packet.PRIMARY_HEADER_PARSED.CASITA_PARSED_AT = Date.now();
