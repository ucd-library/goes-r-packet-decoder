...
class BinaryStreamProcessor {
  ...
  async _runLoop() {
    ...
    while( this.index !== -1 && this.mainbuffer.length > (this.index + CONSTANTS.CADU.PACKET_LENGTH)  ) {
      ...
      if( !this.packet._isValidPacket() ) continue;
  ...
  _writeStats() {
    if( !this.opts.consoleLogStatus ) return;
    if( this.packetCount % 100 !== 0 ) return;
    let packetsPerSecond = this.packetCount / ((Date.now() - this.startTime) / 1000);
    process.stdout.clearLine();  // clear current text
    process.stdout.cursorTo(0);  // move cursor to beginning of line
    process.stdout.write('packets/sec: ' + packetsPerSecond.toFixed() + ', uptime: '+(Date.now() - this.startTime)+'ms');
  }
    ...
    for( let block of blocks ) {
      ...
      if( this.imageBlockOpts.post ) {
        ...
        this._request(params)
          .catch(e => console.log('image payload request error', e))
      }
    }
  ...
  async _handleGenericPayload(generic) {
    ...
    if( this.genericOpts.post ) {
      ...
      this._request(params)
          .catch(e => console.log('generic payload request error', e))
    }
  }
