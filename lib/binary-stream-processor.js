...
class BinaryStreamProcessor {
   ...
   * @param {Function} opts.statusCallback interval callback for status updates
   * @param {Boolean} opts.csvDebug dump packet header information to CSV files
   * @param {Function} opts.onStreamClosed callback for when stream is closed
   ...
   * @param {Object} opts.kafka setup kafka connection for stream processing
   * @param {Object} opts.kafka.client kafka producer client connection config
   * @param {Object} opts.kafka.topic kafka producer topic config
   * @param {Object} opts.kafka.callbacks callbacks to register for node-rdkafka.  Additional produce.error callback for local error
   * @param {Function} opts.preprocessCallback callback before data is sent out.  Function can return a promise
   */
  constructor(opts={}) {
    this.running = false;
    ...
    spSequenceCompositor.registerDataHandler(async msg => {
      if( this.opts.preprocessCallback ) {
        await this.opts.preprocessCallback(msg);
      }
  ...
  async _initOpts(opts) {
    ...
    if( opts.kafka ) {
      this.kafkaClient = new Kafka(opts.kafka.client, opts.kafka.topic, opts.kafka.callbacks);
      this.kafkaConnecting = this.kafkaClient.connect();
      await this.kafkaConnecting;
      this.kafkaConnecting = null;
    }
  }
  ...
  pipe(stream) {
    ...
    stream.on('data', async data => {
      if( (this.opts.consoleLogStatus || this.opts.statusCallback) && !stats.running ) {
        stats.enable({
          consoleLogStatus: this.opts.consoleLogStatus,
          statusCallback: this.opts.statusCallback
        });
      }

      stats.addMBits(data.length / 1000000);
    ...
    stream.on('close', async () => {
      if( this.opts.consoleLogStatus ) console.log('Stream closed');
      spSequenceCompositor.streamClosed = true;
      if( this.opts.csvDebug ) await csvDebug.writeFiles();
      if( this.opts.onStreamClosed ) await this.opts.onStreamClosed();
      process.exit();
    });
  }
  ...
  async _handleImageFragment(image) {
    ...
    if( !this.imageBlockOpts && !this.kafkaClient ) return;
    ...
    for( let block of blocks ) {
      ...
      if( this.kafkaClient && this.kafkaClient.connected ) {
        if( !block.fragments.length ) return;
        ...
        this.kafkaClient.produce({
          ...
          fragmentsCount : block.fragments.length,
          rootMetadata : true,
          imagePayload : block.fragments[0].HEADER,
          spacePacket : block.fragments[0].SPACE_PACKET_HEADER
        });
        ...
        for( let i = 0; i < block.fragments.length; i++ ) {
          ...
          this.kafkaClient.produce(headers, block.fragments[i].DATA);
        }
      }
  ...
  async _handleGenericPayload(generic) {
    if( !this.genericOpts && !this.kafkaClient ) return;
    ...
    if( this.kafkaClient && this.kafkaClient.connected ) {
      this.kafkaClient.produce({
        ...
        type : 'generic',
        streamName : this.opts.name || 'goes-r-stream',
        spacePacketHeaders : generic.spHeaders,
