...
class SpacePacketSequenceCompositor {
  ...
  async process(spacePacket) {
    ...
    // if( id == '91' && spacePacket.packet.PRIMARY_HEADER_PARSED.PACKET_SEQUENCE_COUNT === 14432 ) {
    //   debugger;
    //   console.log('HERE!!');
    // }
  ...
  async emitFinalizedSequence(id, variant) {
    ...
    if( !sequence.valid ) {
      //  if( id == '91' && sequence.invalidReason.match(/14432/) ) {
      //    debugger;
      //   console.log('*****')
      //   console.log('invalid sequence found for: ', id, sequence.invalidReason);
      //   console.log(sequence.packets.map(p => {
      //     return p.packet.PRIMARY_HEADER_PARSED.SEQUENCE_FLAGS+'-'+p.packet.PRIMARY_HEADER_PARSED.PACKET_SEQUENCE_COUNT
      //   }).join(', '))
      //   console.log('*****');
      // }
      global.invalidSequences++;
      return;
    }

    // if( sequence.packets[0].packet.PRIMARY_HEADER_PARSED.SEQUENCE_FLAGS !== 3 ) {
    //   console.log(id, sequence.packets.map(p => {
    //     return p.packet.PRIMARY_HEADER_PARSED.SEQUENCE_FLAGS+'-'+p.packet.PRIMARY_HEADER_PARSED.PACKET_SEQUENCE_COUNT
    //   }).join(', '))
    // }
